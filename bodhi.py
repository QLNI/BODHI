...
class BODHI:
    ...
    # ============================================================
    def _init_db(self):
        # check_same_thread=False lets the web UI (Flask threaded mode) use the
        # same BODHI instance across request threads. Concurrency is still
        # serialised by the chat_ui lock.
    ...
    def do_sleep(self):
        ...
        # Persist every dream from this cycle (not just the primary one).
        # Dream images are regenerated live on demand by chat_ui.dream_image,
        # so we only need to save concept pair + text, not the blended bytes.
        dreams_this_cycle = stats.get("all_dreams") or (
            [stats["dream_content"]] if "dream_content" in stats else []
        )
        for i, dream in enumerate(dreams_this_cycle):
            a, b = dream["a"], dream["b"]
            ...
            # Only attach a primary-dream blended fingerprint for the first
            # dream so the in-process stats still carry it for any caller
            # that wants to display a single dream image.
            if i == 0:
